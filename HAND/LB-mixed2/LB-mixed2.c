...
static void check_globals(ctx_t *_a) {
  ...
  int *x = _a->x;
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+mixed2, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+mixed2, check_globals failed");
  }
...
"sw %[x10],0(%[x12])\n"
"#END _litmus_P1\n\t"
:[x9] "=&r" (out_1_x9[_i]),[x8] "=&r" (out_1_x8[_i]),[x7] "=&r" (out_1_x7[_i]),[x10] "=&r" (trashed_x10),[x5] "=&r" (trashed_x5)
:[x6] "r" (&_a->y[_i]),[x12] "r" (&_a->x[_i])
:"cc","memory"
);
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+mixed2, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+mixed2, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+mixed2 Allowed\n");
  ...
  fprintf(out,"Hash=c6a1c227607ab82dd4a860166a749046\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+mixed2 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  if (p_true > 0) {
  }
  fprintf(out,"Time LB+mixed2 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+mixed2: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
