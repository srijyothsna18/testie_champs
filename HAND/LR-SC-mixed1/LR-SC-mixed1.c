...
inline static int final_cond(int _out_0_x5,int _out_0_x8,int _out_1_x5,int _out_1_x8,uint64_t _x) {
  switch (_out_0_x5) {
  case 0:
    switch (_out_0_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%i; 0:x8=%i; 1:x5=%i; 1:x8=%i; x=%"PRIu64";\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_0_x8_f],(int)o[out_1_x5_f],(int)o[out_1_x8_f],(uint64_t)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  uint64_t *x = _a->x;
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LR-SC-mixed1, check_globals failed");
  }
...
"sc.w %[x8],%[x7],0(%[x9])\n"
"#END _litmus_P1\n\t"
:[x8] "=&r" (out_1_x8[_i]),[x5] "=&r" (out_1_x5[_i]),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->x[_i])
:"cc","memory"
);
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LR-SC-mixed1, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x5_i,_out_0_x8_i,_out_1_x5_i,_out_1_x8_i,_x_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LR-SC-mixed1 Forbidden\n");
  ...
  fprintf(out,"Condition ~exists (x=4294967297 /\\ 0:x5=0 /\\ 0:x8=0 /\\ 1:x5=0 /\\ 1:x8=0) is %svalidated\n",cond ? "" : "NOT ");
  fprintf(out,"Hash=ae5977d2e6f89995161447e584d13045\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LR-SC-mixed1 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  if (p_true > 0) {
  }
  fprintf(out,"Time LR-SC-mixed1 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LR-SC-mixed1: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
    log_error("\n");
  }
  hist_t *hist = NULL;
  int n_th = n_exe-1;
